...
const Dashboard: React.FC = () => {
  ...
  //   setDisCount((prevCount) => prevCount + 1); // Increment count for next call
  // };

  // useEffect(() => {
  //   const intervalId = setInterval(() => {
  //     fetchDistance(disCount); // Fetch data every second
  //   }, 1000);

  //   // Cleanup function to clear interval when component unmounts
  //   return () => clearInterval(intervalId);
  // }, [disCount]); // `disCount` as a dependency isn't necessary for `setInterval`, but okay if needed

  useEffect(() => {
    // 簡單的距離到增益值的計算公式：例如距離越大，增益值增加
